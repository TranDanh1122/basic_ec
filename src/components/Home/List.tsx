                ...
                {type == "product" &&
                    (items as Product[])?.map((el: Product) =>
                        <CarouselItem key={el.id} className="pl-0 basis-[calc(25%-8px)]">
                            <div className="max-h-[250px] w-full aspect-square bg-neutral-300 flex items-center justify-center rounded-sm">
                                <img loading="lazy" src={el.thumbnail} alt={el.title} className="rounded-sm w-[190px] h-[180px] object-contain" />
                            </div>
                            <h3 className="font-medium text-base leading-6 mt-4 mb-2">{el.title}</h3>
                            <span className="font-medium test-base text-[#DB4444]">${el.price}</span>
                        </CarouselItem>
                    )
                }
